...
1. Prepare the **existing 222 open season reviews** with the already previewed 307 AniList title candidates (135 seasons have one, 11 multiple, 19 conflicting, and 57 none). This is private review evidence only. The prepared `apply_review=true` action updates those review rows but must be reviewed before execution; it creates no mapping or watch.
2. Review stable Crunchyroll series and season IDs against approved AniList media IDs and franchise relations. Confirm numbering, seasons, language versions, and the nine parent/episode-series disagreements. A title match by itself never approves a mapping. Look up the one parent series absent from episode panels without guessing an episode for its one history event.
