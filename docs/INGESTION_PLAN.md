...
Crunchyroll coverage update 2026-09-23: [manual run 35931476986](https://github.com/ddibara5/marquee/actions/runs/35931476986) completed using the older `content/v1/watch-history` endpoint's `next_page` links. It reached a reported end after 397 pages and 7,933 observations, from 2023-10-09 to 2026-09-23 UTC. There were 1,000 v1 observations whose event IDs occurred in the `content/v2` probe; confirm one-to-one set equality before relying on this as full overlap. The newer endpoint returned ten full 100-record pages, then HTTP 400 on page 11 even at a smaller page size, so it is not a complete-history source. The v1 feed had 5,979 entries with a panel episode ID and 1,954 without a panel; all 7,933 had parseable `date_played`, but real-world timestamp accuracy and selected-profile scope remain unverified. The probe stores nothing and is not an importer. Before backfill, inspect stable event and episode IDs for missing-panel records, confirm the intended profile, and define the Trakt overlap and mapping-review rules. Do not count missing-panel records as mapped episodes or infer complete lifetime coverage solely from the feed's end marker.
