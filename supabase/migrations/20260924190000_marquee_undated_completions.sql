-- Approved Marquee-only undated historical completion storage, 2026-09-24.
-- New Marquee objects only. This preserves marquee_watch_history.watched_at NOT NULL.
-- Historical source timestamps remain only in marquee_ingest_raw.
  ...
  if new.source = 'crunchyroll' then
    if raw_entity_type <> 'history_event'
       ...
       or raw_payload #>> '{panel,episode_metadata,identifier}'
          is distinct from new.source_episode_identifier
       or raw_payload ->> 'parent_id' in ('GRMG8ZQZR', 'G6DQDD3WR')
       or raw_payload #>> '{panel,episode_metadata,series_id}'
          in ('GRMG8ZQZR', 'G6DQDD3WR') then
