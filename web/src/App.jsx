...
function Bar({ done, total }) {
  return <span className="bar" role="img" aria-label={`${done} of ${total} cataloged episodes complete`}><span style={{ width: `${total ? Math.min(100, done / total * 100) : 0}%` }} /></span>
}
function GearIcon() {
...
function TitleRow({ title, index, onOpen }) {
  ...
  return <button className="title-row" onClick={() => onOpen(title.id)}><Cover title={title}/><span className="row-copy"><strong>{title.display_title}</strong><small>{title.is_anime ? 'Anime' : title.media_type === 'movie' ? 'Film' : 'Series'}{title.release_year ? ` · ${title.release_year}` : ''}</small><span className="row-progress">{title.media_type === 'show' && <Bar {...progress}/>}<span>{title.media_type === 'movie' ? (progress.done ? 'Watched' : 'Unwatched') : progress.total ? `${progress.done} / ${progress.total} cataloged` : 'Episodes pending'}</span></span></span><span className="chevron" aria-hidden="true">›</span></button>
}
function Library({ index, filter, setFilter, query, setQuery, onOpen }) {
  const personal = index.titles.filter(title => index.progress(title).done || index.statuses.has(title.id) || (index.byShow.get(title.id) || []).some(season => index.seasonStatuses.has(season.id)))
  ...
  return <><div className="eyebrow">YOUR PERSONAL COLLECTION</div><h1 className="page-title">Your library.</h1><p className="page-subtitle">Find a story and pick up where you left off.</p>
    {resume && <><div className="section-head"><h2>Up next</h2></div><button className="resume" onClick={() => onOpen(resume.title.id)}><Cover title={resume.title}/><span className="resume-copy"><small>{resume.next ? `Continue · Episode ${resume.next.episode_number ?? 'Special'}` : 'In progress'}</small><strong>{resume.title.display_title}</strong><Bar {...index.progress(resume.title)}/></span><span className="chevron" aria-hidden="true">›</span></button></>}
...
function TitleDetail({ title, index, onSeason, onMovieCheckIn, busy, onSave }) {
  ...
  return <><div className="title-hero"><Cover title={title} large/><div><div className="eyebrow">{title.is_anime ? 'ANIME' : title.media_type === 'movie' ? 'FILM' : 'SERIES'}{title.release_year ? ` · ${title.release_year}` : ''}</div><h1>{title.display_title}</h1>{title.media_type === 'show' && <Bar {...progress}/>}<small>{title.media_type === 'movie' ? (progress.done ? 'Watched' : 'Not watched') : `${progress.done} of ${progress.total} cataloged episodes`}</small></div></div>
    ...
    {title.media_type === 'movie' ? <Evidence history={index.history.get(title.id)} undated={index.movieCoverage.get(title.id)?.has_undated_completion}/> : <><div className="section-head"><h2>Seasons</h2><small>{(index.byShow.get(title.id) || []).length}</small></div><div className="list">{(index.byShow.get(title.id) || []).map(season => { const episodes = index.bySeason.get(season.id) || [], done = episodes.filter(ep => index.coverage.has(ep.id)).length; return <button className="season-row" key={season.id} onClick={() => onSeason(season.id)}><span className="number">{season.season_number === null ? 'SP' : String(season.season_number).padStart(2,'0')}</span><span className="row-copy"><strong>{season.display_title || `Season ${season.season_number ?? 'Specials'}`}</strong><small>{episodes.length ? `${done} of ${episodes.length} cataloged` : statuses[index.seasonStatuses.get(season.id)?.status] || 'Episodes pending'}</small><Bar done={done} total={episodes.length}/></span><span className="chevron" aria-hidden="true">›</span></button> })}</div></>}
  </>
}
function SeasonDetail({ title, season, index, onEpisode, busy, onSave }) {
  const episodes = index.bySeason.get(season.id) || [], done = episodes.filter(ep => index.coverage.has(ep.id)).length
  return <><div className="eyebrow">{title.display_title}</div><h1 className="detail-title">{season.display_title || `Season ${season.season_number ?? 'Specials'}`}</h1><p className="muted">{done} of {episodes.length} cataloged episodes complete</p><Bar done={done} total={episodes.length}/>
