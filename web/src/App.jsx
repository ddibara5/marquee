...
function Bar({ done, total, kind }) {
  if (!total) return null
  return <span className="bar" role="img" aria-label={`${done} of ${total} ${kind === 'released' ? 'released' : 'reviewed season'} episodes complete`}><span style={{ width: `${Math.min(100, done / total * 100)}%` }} /></span>
}
const progressText = progress => progress.total ? `S${progress.season?.season_number ?? '–'} · ${progress.done} of ${progress.total} ${progress.kind}` : `${progress.recorded || 0} episodes recorded`
function GearIcon() {
...
function Library({ index, filter, setFilter, query, setQuery, onOpen }) {
  ...
  return <><div className="eyebrow">YOUR PERSONAL COLLECTION</div><h1 className="page-title">Your library.</h1><p className="page-subtitle">Find a story and pick up where you left off.</p>
    {resume && <><div className="section-head"><h2>Up next</h2></div><button className="resume" onClick={() => onOpen(resume.title.id, resume.season.id, resume.next?.id)}><Cover title={resume.title}/><span className="resume-copy"><small>{resume.next ? `Continue · Episode ${resume.next.episode_number ?? 'Special'}` : `Season ${resume.season.season_number ?? 'specials'} · ${resume.progress.done} of ${resume.progress.total} ${resume.progress.kind}`}</small><strong>{resume.title.display_title}</strong><Bar {...resume.progress}/></span><span className="chevron" aria-hidden="true">›</span></button></>}
...
function TitleDetail({ title, index, onSeason, onMovieCheckIn, busy, onSave }) {
  ...
  return <><div className="title-hero"><Cover title={title} large/><div><div className="eyebrow">{title.is_anime ? 'ANIME' : title.media_type === 'movie' ? 'FILM' : 'SERIES'}{title.release_year ? ` · ${title.release_year}` : ''}</div><h1>{title.display_title}</h1>{title.media_type === 'show' && <Bar {...progress}/>}<small>{title.media_type === 'movie' ? (progress.done ? 'Watched' : 'Not watched') : progressText(progress)}</small></div></div>
    ...
    {title.media_type === 'movie' ? <Evidence history={index.history.get(title.id)} undated={index.movieCoverage.get(title.id)?.has_undated_completion}/> : <><div className="section-head"><h2>Seasons</h2><small>{(index.byShow.get(title.id) || []).length}</small></div><div className="list">{(index.byShow.get(title.id) || []).map(season => { const { done, total, kind } = index.seasonProgress(season); return <button className="season-row" key={season.id} onClick={() => onSeason(season.id)}><span className="number">{season.season_number === null ? 'SP' : String(season.season_number).padStart(2,'0')}</span><span className="row-copy"><strong>{season.display_title || `Season ${season.season_number ?? 'Specials'}`}</strong><small>{total ? `${done} of ${total} ${kind} episodes` : done ? `${done} episodes recorded · total unknown` : statuses[index.seasonStatuses.get(season.id)?.status] || 'Episodes pending'}</small><Bar done={done} total={total} kind={kind}/></span><span className="chevron" aria-hidden="true">›</span></button> })}</div></>}
  </>
}
function SeasonDetail({ title, season, index, onEpisode, busy, onSave }) {
  const episodes = index.bySeason.get(season.id) || [], { done, total, kind, planned } = index.seasonProgress(season)
  return <><div className="eyebrow">{title.display_title}</div><h1 className="detail-title">{season.display_title || `Season ${season.season_number ?? 'Specials'}`}</h1><p className="muted">{total ? `${done} of ${total} ${kind} episodes complete${planned && planned > total ? ` · ${planned} planned` : ''}` : `${done} episodes recorded · season total unknown`}</p><Bar done={done} total={total} kind={kind}/>
